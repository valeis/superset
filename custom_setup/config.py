...
class CustomAuthUserView(AuthRemoteUserView):
    ...
    def login(self):
        ...
        if access_token:
            try:
                payload = jwt.decode(access_token, os.environ.get("SUPERSET_SECRET_KEY"), algorithms=["HS256"])
                userID = payload.get("sub")
                logger.info(f'User {userID} found, logging in...')

                session = sm.get_session
                user = session.query(sm.user_model).filter_by(id=userID).first()
                if user:
                ...
                logger.info(f'Access token has expired')
                # Optional: Check refresh token here and issue a new access token if valid
            except jwt.InvalidTokenError:
                logger.info(f'Invalid access token')
        else:
            flash('No access token provided', 'warning')
...
ENABLE_CORS = True
HTTP_HEADERS = {
    'Authorization': f'Bearer BiMJbGJthWHmBOKfYII8zntacJM3ETv1sgoO3cmJiR4NZ8OdWehNsisV'  # Add the Authorization header
}
CORS_OPTIONS = {
